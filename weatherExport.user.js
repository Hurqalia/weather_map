// ==UserScript==
// @author         jonatkins
// @name           IITC plugin: Ingress scoring regions exporter
// @category       Layer
// @version        0.3.0
...
function wrapper(plugin_info) {
	...
	//plugin_info.dateTimeVersion = '20200702.001';
	//plugin_info.pluginId = 'regionsExport';

// use own namespace for plugin
window.plugin.regionsExport = function() {};

window.plugin.regionsExport.setup  = function() {
...
;
  ...
  window.plugin.regionsExport.regionLayer = L.layerGroup();
  ...
  addLayerGroup('Score Regions Export', window.plugin.regionsExport.regionLayer, true);

  map.on('moveend', window.plugin.regionsExport.update);

  addHook('search', window.plugin.regionsExport.search);

  window.plugin.regionsExport.update();
};

window.plugin.regionsExport.FACE_NAMES = [ 'AF', 'AS', 'NR', 'PA', 'AM', 'ST' ];
window.plugin.regionsExport.CODE_WORDS = [
...
window.plugin.regionsExport.REGEXP = new RegExp('^(?:(?:(' + plugin.regionsExport.FACE_NAMES.join('|') + ')-?)?((?:1[0-6])|(?:0?[1-9]))-?)?(' +
  plugin.regionsExport.CODE_WORDS.join('|') + ')(?:-?((?:1[0-5])|(?:0?\\d)))?$', 'i');

window.plugin.regionsExport.regionName = function(cell) {
  ...
  var name = window.plugin.regionsExport.FACE_NAMES[cell.face];
  ...
  if (cell.level >= 4) {
    ...
    name += zeroPad(regionI+1,2)+'-'+window.plugin.regionsExport.CODE_WORDS[regionJ];
  }
...
window.plugin.regionsExport.search = function(query) {
  var terms = query.term.replace(/\s+/g, '').split(/[,;]/);
  var matches = terms.map(function(string) {
    return string.match(window.plugin.regionsExport.REGEXP);
  });
  if(!matches.every(function(match) { return match !== null; })) return;
  
  var currentCell = window.plugin.regionsExport.regionName(S2.S2Cell.FromLatLng(map.getCenter(), 6));
  ...
  matches.forEach(function(match) {
    ...
    var result = window.plugin.regionsExport.getSearchResult(match);
    if(result) query.addResult(result);
  });
}

// rot and d2xy from Wikipedia
window.plugin.regionsExport.rot = function(n, x, y, rx, ry) {
  if(ry == 0) {
    if(rx == 1) {
      x = n-1 - x;
      y = n-1 - y;
...
window.plugin.regionsExport.d2xy = function(n, d) {
  var rx, ry, s, t = d, xy = [0, 0];
  for(s=1; s<n; s*=2) {
    rx = 1 & (t/2);
    ry = 1 & (t ^ rx);
    xy = window.plugin.regionsExport.rot(s, xy[0], xy[1], rx, ry);
    xy[0] += s * rx;
    xy[1] += s * ry;
    t /= 4;
  }
...
window.plugin.regionsExport.getSearchResult = function(match) {
  var faceId = window.plugin.regionsExport.FACE_NAMES.indexOf(match[1]);
  var id1 = parseInt(match[2]);
  var codeWordId = window.plugin.regionsExport.CODE_WORDS.indexOf(match[3]);
...
');
    ...
    var xy = window.plugin.regionsExport.d2xy(4, id2);
  ...
  result.title = window.plugin.regionsExport.regionName(cell);
...
window.plugin.regionsExport.update = function() {

  window.plugin.regionsExport.regionLayer.clearLayers();
  ...
  var drawCellAndNeighbors = function(cell) {
    ...
    if (!seenCells[cellStr]) {
      ...
      if (cellBounds.intersects(bounds)) {
        // on screen - draw it
        window.plugin.regionsExport.drawCell(cell);
  ...
  for (var i=0; i<latLngs.length-1; i++) {
    ...
    window.plugin.regionsExport.regionLayer.addLayer(poly1);
    ...
    window.plugin.regionsExport.regionLayer.addLayer(poly2);
  }
  ...
  for (var i=-135; i<=135; i+=90) {
    ...
    window.plugin.regionsExport.regionLayer.addLayer(poly);
  }
}

window.plugin.regionsExport.drawCell = function(cell) {
...
//TODO: move to function - then call for all cells on screen
  ...
  // name
  var name = window.plugin.regionsExport.regionName(cell);

  if(!window.selectedCells) window.selectedCells = new Map();
  var color = window.selectedCells.has(name) ? 'red' : 'gold';
  // the level 6 cells have noticible errors with non-geodesic lines - and the larger level 4 cells are worse
  // NOTE: we only draw two of the edges. as we draw all cells on screen, the other two edges will either be drawn
  // from the other cell, or be off screen so we don't care
  var region = L.geodesicPolygon([corners[0],corners[1],corners[2],corners[3],corners[0]], {cellname: name, fill: true, color: color, opacity: 0.5, weight: 5, interactive: true });
  var clickHandler = function(e) {
      if (window.selectedCells.has(name)) {
          window.selectedCells.delete(this.options.cellname);
          this.options.color = 'gold';
      } else {
          window.selectedCells.set(this.options.cellname, 
            {
                name: this.options.cellname, 
                corners: 
                    [
                        {'lat': this._latlngsinit[0].lat, 'lng': this._latlngsinit[0].lng},
                        {'lat': this._latlngsinit[1].lat, 'lng': this._latlngsinit[1].lng},
                        {'lat': this._latlngsinit[2].lat, 'lng': this._latlngsinit[2].lng},
                        {'lat': this._latlngsinit[3].lat, 'lng': this._latlngsinit[3].lng},
                    ]
            });
  ...
  window.plugin.regionsExport.regionLayer.addLayer(region);
...
var setup =  window.plugin.regionsExport.setup;
